...
def makeVarPlot(hfile, hname, tag, title, collection, names, color=r.kBlue, ylog=True):
    ...
    if data == 'AOD':
        ...
        latex.DrawLatex(0.60, 0.83, "With emulated DisplacedMuonFilter".format(data))
...
def makeNMatchesPlot(hfile, tag, ylog=True):
    ...
    if data == 'AOD':
        ...
        latex.DrawLatex(0.17, 0.86, "NoBPTX3BX trigger, no ID".format(data))
...
def make2DNMatchesPlot(hfile, title, tag, zlog=False):

    h = hfile.Get('h_nmatches_nsegments_2D_dmu_dsa')

    c = r.TCanvas('h_nmatches_nsegments_2D_dmu_dsa','h_nmatches_nsegments_2D_dmu_dsa',ww=620,wh=600)
    c.SetFillStyle(4000)
    c.cd()
    if zlog: c.SetLogz(1)
    else: c.SetLogz(0)
    ...
    if not os.path.exists(EOSPATH+'Plots/'+tag):
        os.makedirs(EOSPATH+'Plots/'+tag)
    c.Print(EOSPATH+'Plots/'+tag+'/h_nmatches_nsegments_2D_dmu_dsa.png')
    c.Print(EOSPATH+'Plots/'+tag+'/h_nmatches_nsegments_2D_dmu_dsa.pdf')
...
if __name__ == '__main__':
    ...
    collection = 'dsa'
    makeVarPlot(hfile, 'h_dxy', args.tag, title, collection, names=['track({0})'.format(collection),'muon({0})'.format(collection)], color=r.kRed+1, ylog=False)
    ...
    make2DNMatchesPlot(hfile, title, args.tag, zlog=False)
